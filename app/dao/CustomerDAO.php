...
class CustomerDAO
{
    ...
    /**
     * Update customer profile. If password is null or empty, the password column is not changed.
     */
    protected function update(Customer $customer)
    {
        if (!empty($customer->getPassword())) {
            ...
            $query = "UPDATE customer SET fullname = :fullname, phone = :phone WHERE cid = :cid;";
            $stmt = DB::getInstance()->prepare($query);
            $exec = $stmt->execute([
                'fullname'  => $customer->getFullName(),
                'phone'     => $customer->getPhone(),
