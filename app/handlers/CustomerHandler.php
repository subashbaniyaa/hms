...
class CustomerHandler extends CustomerDAO
{
    ...
    /**
     * Returns an array of Customer objects for the given email.
     * Returns an empty array (never a string) if not found or on DB error.
     */
    ...
    {
        $rows = $this->getSingleRow($customer->getEmail());
        if (empty($rows)) {
            return false;
        }
        $cust = $rows[0];
